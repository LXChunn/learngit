...
- (void)viewDidLoad
{
    ...
    [self.phoneTextField rac_textSignalWithLimitLength:11];
    [self.verificationCodeTextField rac_textSignalWithLimitLength:6];
    ...
    [[[RACObserve(self.viewModel, model) ignore:nil] take:1] subscribeNext:^(id x) {
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            if(![XPLoginModel singleton].isBound) {
                ...
                [[UIViewController getCurrentVC] presentViewController:house animated:YES completion:^{
                }];
            }
        }];
    }];
